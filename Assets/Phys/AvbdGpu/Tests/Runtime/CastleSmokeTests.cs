...
namespace Phys.AvbdGpu.Tests
{
    ...
    public class CastleSmokeTests
    {
        ...
        public IEnumerator CannonballKnocksBricksOff()
        {
            ...
            float cube = 0.15f * s;
            int ball = m_Demo.World.AddBody(new float3(cube), m_Demo.ShotMass / (cube * cube * cube), 0.5f, from, quaternion.identity, math.normalize(target - from) * 12f * math.sqrt(s));
            ...
            Assert.Less(moved, m_Demo.BrickCount / 4, "the castle did not collapse entirely");
        }

        [UnityTest]
        public IEnumerator SnappedCastleBuildsJoints()
        {
            m_Demo.Snap = true;
            m_Demo.Load(0);
