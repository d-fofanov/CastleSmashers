...
namespace Phys.AvbdGpu.Tests
{
    ...
    {
        static void Measure(string label, int scene, int bodies, int iterations, int warmup, int steps) => Measure(label, w => w.BuildScene(scene), bodies, iterations, warmup, steps);

        static void Measure(string label, System.Action<AvbdGpuWorld> build, int bodies, int iterations, int warmup, int steps)
        {
            ...
            UnityEngine.Debug.Log($"PERF {label}: {world.BodyCount} bodies, {iterations} iterations: {sw.Elapsed.TotalMilliseconds / steps:F2} ms/step, cpu submit {world.Stats.AvgStepMs:F2} ms " +
                $"(pairs {stats.Pairs}, manifolds {stats.Manifolds} ({stats.TerrainManifolds} terrain), contacts {stats.Contacts}, colours {stats.ColorsUsed}, overflow bodies {stats.OverflowBodies}, flags {stats.OverflowFlags})");
